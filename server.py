...
class ChatServerHost:
    ...
    def message_sender(self, message, specified_client_connection=None):  # Send a message to a client
        if specified_client_connection:  # Whisper
            ...
            for client in self.clients.values():  # Send to all active clients
            ...
            try:
                if len(message.split()) > 2:  # Client entered a nickname consisting of more than 1 word
                    ...
                    if nickname in self.names:  # Nickname is occupied
                        ...
                        save_users(self.users)  # Updates json
                        ...
                        sleep(0.1)  # Sleep var nödvändig för att
    ...
    def client_handler(self, client_username, client_info):  # Loop to be threaded for every client
        ...
        while not disconnected:
            try:
                ...
                disconnected_user = client_info[
                    'name']  # User must be saved so message can be sent after user termination
    ...
    def login(self, client_connection):  # Login function to access your profile
        ...
        while not logged_in:
            try:
                ...
                username, password, action = login_info.split()  # Separates information
                ...
                elif username in self.users:  # Creating new account, username already occupied
                    self.message_sender("Username not available", client_connection)
                else:  # Creating new account, username not occupied
                    user = {
                        "password": password,
    ...
    def update_active_users(self):  # Used for client to display active clients
    ...
    def connections(self):  # Function to accept new connections
        while True:
            client_connection = self.server_socket.accept()[0]  # Accepts new client
            self.login(client_connection)  # Runs login for client

    def start_server(self):  # Server startup
        self.handle_saved_users(get_users())  # Get user data
        self.connections()  # Start looking for connections
